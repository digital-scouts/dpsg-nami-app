...
Mitgliedsliste, Suche, Statistik und persönliche Dashboards arbeiten jeweils nur innerhalb des aktiven Arbeitskontexts und nur auf dem aus Hitobito lesbar geladenen Bestand. Für den MVP wird die In-App-Stufe global im Code über zentrale Regeln zu Hitobito-Gruppentypen abgeleitet. Neben den Stufenfiltern stehen benutzerdefinierte Filtergruppen zur Verfügung, die Gruppen- und Rollenzuordnungen innerhalb des aktiven Arbeitskontexts kombinieren. Die Mitgliedsliste zeigt ohne ausgewählten Chip alle lesbaren Personen; Stufen- und eigene Gruppen-Chips schränken diese Menge nur als Filter ein und wechseln nie den Arbeitskontext. Über "Filtern & Sortieren" können Sortierung, Zusatztext und eigene Gruppen angepasst werden; Änderungen werden erst mit "Anwenden" gespeichert. Die Standardgruppe "Rest" bildet Personen ohne Stufenzuordnung ab. Leere Gruppen ohne Personen werden in der Leseansicht nicht angezeigt. Leere Layer bleiben dagegen als mögliche Arbeitskontexte zulässig. Ohne mindestens ein relevantes Layer- oder Gruppen-Lese- beziehungsweise Schreibrecht zeigt die App für kontextabhängige Bereiche einen expliziten Nicht-berechtigt-Zustand mit Logout an. Stamm-Einstellungen bleiben davon ausgenommen erreichbar.
