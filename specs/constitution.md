...
- Sync-Strategie aktuell: Cache zuerst für Lesedaten. Remote-Updates für Hitobito-Daten werden regulär nur bei fälligem Intervall (`HITOBITO_REFRESH_INTERVAL_HOURS`) versucht. Während aktiver App-Nutzung darf zusätzlich ein Foreground-Sync angestoßen werden, wenn Netzwerkzugriff nach WLAN-Verfügbarkeit oder nach Freigabe mobiler Daten wieder erlaubt ist. Schlägt ein Update fehl, bleibt der vorhandene Cache lesbar und ein fachlicher Hinweis wird angezeigt. Änderungen starten gegen frische Remote-Daten; retrybare Sendefehler werden lokal vorgemerkt und bei wieder erlaubtem Netzwerkzugriff während aktiver Nutzung automatisch erneut versucht.
