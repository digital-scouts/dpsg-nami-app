...
Im Zuge der 3.7-Planung gesammelte Ideen, bewusst nicht Teil dieser Phase, aber bei den 3.7-Designentscheidungen offen gehalten (keine davon wird durch 3.7 blockiert):

- Vergangene Unterhaltungen wieder öffnen und mit Kontext Rückfragen stellen, statt nur read-only nachzulesen (3.7 speichert bewusst nur zum Nachlesen, siehe 3.7-Persistenzdesign).
- Manuelles Löschen einzelner Chat-Historie-Einträge (3.7 kennt nur automatischen 30-Tage-Ablauf).
- Chats/Unterhaltungen logisch gruppieren (z. B. alle Fragen zur Stammesversammlung in einem Ordner/Tag).
- Themenwechsel-Erkennung innerhalb einer laufenden Session: wenn der Nutzer erkennbar das Thema wechselt, kurzer Hinweis mit Wahl „hier weiter" / „neue Unterhaltung starten" statt stillschweigend im selben Kontext weiterzumachen.
- Weitere Dokumente für globalen Kontext (z. B. Interventionsordnung), dynamischer Kontext je nach aktivem Arbeitskontext (z. B. DV-spezifische Präventionsordnung), sowie eine explizite Regelwerk-Hierarchie bei widersprüchlichen Quellen (vgl. offene Frage in Abschnitt 7 „Wie wird bei widersprüchlichen Quellen priorisiert").
- Quellenangaben unter der Antwort interaktiv/nachvollziehbar machen: Tippen auf eine Quelle öffnet den zugehörigen Absatz (3.7 liefert nur eine kompakte, nicht-interaktive Quellenzeile).
- Gezielte Halluzinationskontrolle für Reasoning-Anfragen (z. B. „Darf der Stammesvorstand X ohne Stammesversammlung entscheiden?"), bei denen die Antwort mehrere Quellen fachlich korrekt verknüpfen muss, ohne Fakten zu erfinden — über das bestehende Grounding-Gate (3.6) hinaus.
- Antwort-Validator/Verifier-Pass: ein zweiter Modelldurchlauf prüft die generierte Antwort gegen die Quellen, bevor sie an den Nutzer geht.
...
- Function Calling: der Chat löst App-Funktionen aus statt nur Fragen zu beantworten, mit klar definierten Tool-Grenzen (Rechte-/Rollenprüfung vor jeder Aktion) — Kernidee von Meilenstein M3.
- KI-generierte Kurzzusammenfassung als Chat-Titel statt der ersten Nutzerfrage (3.7 nutzt bewusst die erste Frage, um keinen zusätzlichen Modellaufruf/Fehlerfall einzuführen).
- Live-Kontext-Meter mit Session-Ende bei vollem Kontext statt der in 3.7 gewählten stillen Sliding-Window-Truncation mit einmaligem Hinweis danach — abhängig davon, ob FoundationModels künftig einen belegten Weg bietet, Kontextverbrauch live auszulesen.
