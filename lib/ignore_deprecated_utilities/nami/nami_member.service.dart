...
import 'package:nami/ignore_deprecated_utilities/nami/model/nami_member_ausbildung_model.dart';
import 'package:nami/ignore_deprecated_utilities/nami/nami.service.dart';
import 'package:nami/ignore_deprecated_utilities/nami/nami_member_fake.service.dart';
import 'package:nami/ignore_deprecated_utilities/nami/nami_rechte.dart';
import 'package:nami/ignore_deprecated_utilities/types.dart';

import 'model/nami_member_details.model.dart';
import 'model/nami_taetigkeiten.model.dart';

/// NamiMemberService - Refactored Service für die Verwaltung von Mitgliederdaten
///
/// Diese Datei wurde von einer funktionalen Struktur zu einem objekt-orientierten
/// Service-Pattern refactoriert. Die wichtigsten Änderungen:
///
/// 1. **Service-Klasse**: Alle Funktionen sind nun Methoden der NamiMemberService-Klasse
/// 2. **HiveService-Integration**: Der Service verwendet nun den HiveService für
///    alle Datenbankoperationen
/// 3. **Dependency Injection**: HiveService und DataChangesService werden als
///    Dependencies injiziert
/// 4. **Rückwärtskompatibilität**: Legacy-Funktionen sind als Wrapper-Funktionen
///    erhalten, die die neue Service-Instanz verwenden
/// 5. **Listener-Support**: Der Service nutzt die Listener-Funktionalität des HiveService
///
/// Die globale Service-Instanz `namiMemberService` kann direkt verwendet werden,
/// oder es können eigene Instanzen mit Mock-Dependencies für Tests erstellt werden.
///
/// Beispiel für Tests:
/// ```dart
/// final testService = NamiMemberService(
///   hiveService: mockHiveService,
///   dataChangesService: mockDataChangesService,
/// );
class NamiMemberService {
  final HiveService _hiveService;
  final DataChangesService _dataChangesService;

  NamiMemberService({
    HiveService? hiveService,
    DataChangesService? dataChangesService,
  }) : _hiveService = hiveService ?? HiveServiceImpl(),
       _dataChangesService = dataChangesService ?? DataChangesService();

  /// Getter für die Mitglieder-Box
  Box<Mitglied> get memberBox => _hiveService.memberBox;

  /// Hilfsfunktion um die Version eines Mitglieds zu ermitteln
  ...
  /// Lädt die IDs der zu aktualisierenden Mitglieder
  Future<Map<int, int>> _loadMemberIdsToUpdate(
    ...
    List<Mitglied> mitglieder = memberBox.values.toList().cast<Mitglied>();
    ...
    for (var mitglied in mitglieder) {
      if (!loadedMemberIds.contains(mitglied.id)) {
        ...
        _dataChangesService.addDataChangeEntry(
          mitglied.id!,
          action: DataChangeAction.delete,
        );
        await memberBox.delete(mitglied.id);
      }
    }
    return memberIds;
  }

  /// Lädt die Mitgliederdetails von NaMi
...
final NamiMemberService namiMemberService = NamiMemberService();

// Legacy-Funktionen für Rückwärtskompatibilität
Future<Mitglied> updateOneMember(int memberId) async {
  return namiMemberService.updateOneMember(memberId);
}
...
Future<void> syncMembers(
  ...
  return namiMemberService.syncMembers(
    memberAllProgressNotifier,
    memberOverviewProgressNotifier,
...
Future<void> endMembership(int memberId, DateTime endDate) async {
  return namiMemberService.endMembership(memberId, endDate);
}
...
Future<int?> findUserId(
  ...
  return namiMemberService.findUserId(memberId, mitgliedIds);
}

DateTime getEarliestStartDate(List<Taetigkeit> taetigkeiten) {
  return namiMemberService.getEarliestStartDate(taetigkeiten);
}
